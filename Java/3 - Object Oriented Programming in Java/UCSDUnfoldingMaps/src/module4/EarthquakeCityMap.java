...
public class EarthquakeCityMap extends PApplet {
	...
	private void addKey() {	
		...
		text("Landquake", 75, 145);
		text("Oceanquake", 75, 165);
	...
	private boolean isLand(PointFeature earthquake) {
		...
		for (Marker m : countryMarkers) {
			...
			boolean pres = isInCountry(earthquake, m);
			if(pres) {
				return pres;
			}
		}
	...
	private void printQuakes() 
	{
		...
		HashMap<String, Integer> quakeCountryMap = new HashMap<String, Integer>();
		quakeCountryMap.put("Ocean", 0);
		for(Marker m: quakeMarkers) {
			EarthquakeMarker em = (EarthquakeMarker)m;
			if(em.isOnLand()) {
				String country = (String)em.getProperty("country");
				if(quakeCountryMap.containsKey(country)) {
					quakeCountryMap.put(country, quakeCountryMap.get(country) + 1);
				}
				
				else {
					quakeCountryMap.put(country, 1);
				}
			}
			
			else {
				quakeCountryMap.put("Ocean", quakeCountryMap.get("Ocean") + 1);
			}
		}
		
		for(String country: quakeCountryMap.keySet()) {
			System.out.println(country + ": " + quakeCountryMap.get(country));
