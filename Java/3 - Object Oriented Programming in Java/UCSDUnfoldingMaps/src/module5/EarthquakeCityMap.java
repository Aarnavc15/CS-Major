...
public class EarthquakeCityMap extends PApplet {
	...
	private void selectMarkerIfHover(List<Marker> markers)
	{
		for(Marker m: markers) {
			boolean value = m.isInside(map, this.mouseX, this.mouseY);
			m.setSelected(false);
			if(value) {
				m.setSelected(true);
	...
	public void mouseClicked()
	{
		...
		if (this.lastClicked != null) {
			...
			Marker m = this.lastClicked;
			if(cityMarkers.contains(m)) {
				for(Marker lol: cityMarkers) {
					if(!lol.equals(m)) {
						lol.setHidden(true);
					}
				}
				for(Marker q: quakeMarkers) {
					...
					if(quake.threatCircle() >= m.getDistanceTo(quake.getLocation())){
			...
			else if(quakeMarkers.contains(m)) {
				for(Marker lol: quakeMarkers) {
					if(!lol.equals(m)) {
						lol.setHidden(true);
					}
				}
				...
				for(Marker c: cityMarkers) {
					if(quake.threatCircle() >= c.getDistanceTo(quake.getLocation())){
						c.setHidden(false);
					}
					else {
						c.setHidden(true);
